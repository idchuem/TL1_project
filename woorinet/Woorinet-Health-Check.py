...
def send_mail(saveFile,text):
    ...
    addrs =['snocjs@skbroadband.com']
    subject = "우리넷 시스템 버전 조회"
    text ='노드접속불가 리스트(timed out): '+text
    ...
    with open(saveFile, "rb") as file:
        part = MIMEApplication(file.read(), Name=basename(saveFile))
        part['Content-Disposition'] = 'attachment; filename="%s"' % basename(saveFile)
        msg.attach(part)
...
class Woorinet:
    ...
    # make instance and connect TL1 device
    def __init__(self, ip, id, pw):
        self.ne_ip = ip
        self.ne_id = id
        self.ne_pw = pw
        self.tl1 = Telnet(ip, port=5543)
        self.ne_login()

    # login to TL1 device
    def ne_login(self):
        self.tl1.read_until(':')
        self.tl1.write(self.ne_id + '\n')
        self.tl1.read_until(':')
        self.tl1.write(self.ne_pw + '\n')
        self.tl1.write(';\n')
        print('Successfully logged in')

    def ne_logout(self):
        print('Logging out')
        self.tl1.write('CANC-USER::ADMIN:CTAG;')
        self.tl1.close()
        print('Logged out from ' + self.ne_ip)
    ...
    def get_sysname(self):
        self.tl1.write(';\n')
        self.tl1.read_until(';')
        self.tl1.write('RTRV-SYS:::;\n')
        log = self.tl1.read_until(';')
        ne_neid = log[8:15]
        val = self.get_logs(ne_ip, log)
        val = val.split(',')
        # |  val[0] |   val[1]  | vspal[2] |  val[3]
        # |   TID   |  Location | Vendor | DateTime
        value = val[0] + ',' + val[1]
        sys_result= {'TID': val[0], }
        return sys_result

    def get_model(ne_ip,self.tl1):
        self.tl1.write(';\n')
        self.tl1.read_until(';')
        self.tl1.write('RTRV-DATE:::;\n')
        Log = self.tl1.read_until(';')
    ...
    ############################################################################################
    def get_lowcapa(ne_ip, self.tl1):
        self.tl1.write('RTRV-LO-CAPA:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        val = val.split(',')
    ...
    ############################################################################################
    def get_swversion(ne_ip, self.tl1):
        self.tl1.write('RTRV-VER-SW:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        val = val.split(',')
        # |     val[0]    |    val[1]     |  val[2] |  val[3]
    ...
    ############################################################################################
    def get_clkconfig(ne_ip, self.tl1):
        self.tl1.write('RTRV-CLK:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        val = val.split(',')
    ...
    ############################################################################################
    def get_clkderived(ne_ip, self.tl1):
        self.tl1.write('RTRV-CLK-DERIVED:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        val = val.split(',')
        # |  val[0] |  val[1] |  val[2] | val[3] |
    ...
    # Returns clock ssm
    ############################################################################################
    def get_clkssm(ne_ip, self.tl1):
        self.tl1.write('RTRV-SSM:::;\n')
        Log = self.tl1.read_until(';')
        val=woorinet.get_clkconfig(ne_ip,self.tl1)
        clkcfg=val.split(',')
    ...
    ############################################################################################
    def get_portstate(ne_ip, AID, self.tl1):
        self.tl1.write('RTRV-PORT::' + AID + ':;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        # |   [0]    |    [1]               | [3] #
    ...
    # Returns slot states
    ############################################################################################
    def get_slotstate(ne_ip, self.tl1):
        self.tl1.write('RTRV-SLOT:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        # |   [0]     |  [1]   | #
        # | SLOT:UNIT | status | #
    ...
    ############################################################################################
    def get_mspfailure(ne_ip, self.tl1):
        self.tl1.write('RTRV-SW:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        # |  val[0]  |   val[1]   | val[2] |  val[3]  |  Val[4] | ~~~~
        ...
        val = val.split(',')
        value = ""
        alms = woorinet.get_alms(ne_ip, self.tl1)  # 모든 ALM 긁어오기
        # |    val[0]    |  val[1]  |  val[2] |  val[3]  |  Val[4] | val[5]| ~~~~
        # | AID:Severity |  SA/NSA  |  TYPE   |  Reason  |  Date   | Time  | ~~~~
        for i in range(len(val)):
            if val[i] == 'P-FAIL':  # Protection Fail인데
                port = val[i - 2].split(':')[0]  # P-Fail 포트 선언
                act = woorinet.get_portstate(ne_ip, port, self.tl1)  # P-Fail포트가 ACT인지 확인
    ...
    ############################################################################################
    def get_mssprfailure(ne_ip, self.tl1):
        self.tl1.write('RTRV-SW-MSSPR:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        # |  val[0]  |     val[1]   |     val[2]    |   val[3]  |  Val[4] | Val[5]  | ~~~~
    ...
    ############################################################################################
    '''
    def get_tca(ne_ip, self.tl1):
        slot=[]
        val = woorinet.get_slotstate(ne_ip,self.tl1)
        val1 = val.split(',')
        value=""
        #print(val1)
        ...
        for i in range(len(slot)):
            self.tl1.write('RTRV-PM-RS::'+slot[i]+':;\n')
            Log = self.tl1.read_until(';')
            val = woorinet.get_logs(ne_ip, Log)
            print(val)
            val=val.split('-')
    ...
    # Return alarms
    ############################################################################################
    def get_alms(ne_ip, self.tl1):
        self.tl1.write('RTRV-ALM:::;\n')
        Log = self.tl1.read_until(';')
        val = woorinet.get_logs(ne_ip, Log)
        value = ''
        val = val.split(',')
    ...
    ############################################################################################
    def analyze_alarm(ne_ip, self.tl1):
        val = get_alms(ne_ip, self.tl1)
        # |    val[0]    |  val[1]  |  val[2] |  val[3]  |  Val[4] | val[5]| ~~~~
        # | AID:Severity |  SA/NSA  |  TYPE   |  Reason  |  Date   | Time  | ~~~~
        val = val.split(',')
        value = ""
...
if __name__ == '__main__':
    ...
    IP = ne_ip("woorinet_neip")

    for i in range(len(IP)):  # IP수 만큼 함수 실행
        try:
            self.tl1,neid = woorinet.login(IP[i])  # TL1접속
            #perp=woorinet.get_performance(IP[i],self.tl1)
            # 시스템 정보 조회
            sysname = woorinet.get_sysname(IP[i], self.tl1)
            ...
            #모델명 조회
            model=woorinet.get_model(IP[i],self.tl1)
            MODEL.append(model)

            # MCU 버전 조회
            val = woorinet.get_swversion(IP[i], self.tl1)
            val = val.split(',')
            SLOT.append(val[0])
            VER.append(val[1])
            ...
            """
            # Low Order 용량 조회
            val = woorinet.get_lowcapa(IP[i], self.tl1)
            val = val.split(',')
            AU3Usage.append(val[0])
            AU4Usage.append(val[1])
            
            # Clock원 조회
            val = woorinet.get_clkconfig(IP[i], self.tl1)
            val = val.split(',')
            CURCLK.append(val[0])
            PRVCLK1.append(val[1])
            PRVCLK2.append(val[2])
            # Clock원 품질조회
            val=woorinet.get_clkssm(IP[i], self.tl1)
            val = val.split(',')
            CURSSM.append(val[1])
            CLKSSMPRV.append(val[2])
    
            #유도클럭원 조회
            val =woorinet.get_clkderived(IP[i], self.tl1)
            val = val.split(',')
            CLKCUR.append(val[0])
            CLKSSM.append(val[1])
    
            # MSP-SW FAIL 조회
            val = woorinet.get_mspfailure(IP[i], self.tl1)
            SWFAIL.append(val)
            # MSSPR FAIL 조회
            val = woorinet.get_mssprfailure(IP[i], self.tl1)
            MSSPRFAIL.append(val)
            """
        except:
            timed_out.append(IP[i])
    """
    ...
    result = {'노드명': TID, '모델명':MODEL, 'IP주소': NEIP, 'NE_ID': NEID, '설치위치': Location,'Working MCU': SLOT, 'MCU버전': VER}
    ...
    for ip in timed_out:
        text += ip+', '
    saveFile="c:/intel/woorinet_version.csv"
    df.to_csv(saveFile, index="False", encoding='ms949')
    send_mail(saveFile,text)
    print(' Completed')